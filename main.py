''' MINI PROJECT
Countdown timer in python with
Start/Stop
and Pause/Reset Button'''
import time
from tkinter import *
...
class CountDown:
    def __init__(self, root):
        self.window = root
        #win.geometry(format(window_width, window_height, x_cordinate, y_cordinate))
        self.window.geometry("480x320+539+120")
        self.window.title('Countdown Timer')
        # Window background color
        self.window.configure(bg='#00c4cc')
        ...
        # The Start and Pause buttons are placed inside this frame
        self.button_frame = Frame(self.window, bg="#00c4cc", \
                                  width=240, height=40)
        self.button_frame.place(x=230, y=150)
        # This frame is used to show the countdown time label
        self.time_frame = Frame(self.window, bg="#00c4cc", \
                                width=480, height=120).place(x=0, y=210)

        # Tkinter Labels
        time_label = Label(self.window, text="Set Time",
                           font=("times new roman", 22, "bold"), bg='#00c4cc', fg='gray35')
        time_label.place(x=180, y=30)

        hour_label = Label(self.window, text="Hour",
                           font=("times new roman", 15), bg='#00c4cc', fg='white')
        ...
        # Tkinter Buttons
        # Cancel button
        cancel_button = Button(self.window, text='Quit',
                               font=('Helvetica', 12), bg="white", fg="black",
                               command=self.Cancel)
        cancel_button.place(x=50, y=150)
    ...
    def Reset_Time(self):
        ...
        messagebox.showinfo('Reset', 'Reset done')
